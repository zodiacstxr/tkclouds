...
class GoogleDriveApi{
  ...
  public function __construct($strClientId, $strClientSecret){
    ...
    $this->strScriptUrl = "{$_SERVER['REQUEST_SCHEME']}://{$_SERVER['SERVER_NAME']}{$_SERVER['PHP_SELF']}";
  }
  ...
  public function Download($strFileId,$fileName,$mime_type){
    // Delete File and Folder From Id
    $strUrl = $this->strApiUrl."/files/".$strFileId."?alt=media";
  ...
  public function Upload($parentsId="root", $file){
    ...
    preg_match_all('@((https?://)?([-\\w]+\\.[-\\w\\.]+)+\\w(:\\d+)?(/([-\\w/_\\.]*(\\?\\S+)?)?)*)@',$result,$arrUploadLinkUrl);
    //echo '<br/><br/>';
    //print_r($arrUploadLinkUrl[0][0]);
    //$strUploadLinkUrl = $arrUploadLinkUrl[1][0];
    $strUploadLinkUrl = $arrUploadLinkUrl[0][0];
    // Upload File
    $headers = array();
    $headers[] = "Content-Type: application/json";
    $headers[] = "Authorization: OAuth ".urlencode($this->strAccessToken);
